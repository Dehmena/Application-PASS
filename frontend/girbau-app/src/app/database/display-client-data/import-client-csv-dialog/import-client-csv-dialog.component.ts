...
export interface ClientDataImportInterface {
    number: number;
    name: string;
    mail: string;
}
...
@Component({
  selector: 'app-import-client-csv-dialog',
  templateUrl: './import-client-csv-dialog.component.html',
  styleUrls: ['./import-client-csv-dialog.component.css']
})
export class ImportClientCsvDialogComponent implements OnInit {

    public csvFile: File;
    public dataToInsert: ClientDataImportInterface[] = [];
    public dataToUpdate: ClientDataImportInterface[] = [];
    ...
    public conflictedData: ClientDataImportInterface[] = [];
    ...
    constructor(public dialogRef: MatDialogRef<ImportClientCsvDialogComponent>,
    ...
    processCsv() {
        ...
        this.http.post(AppSettings.URL_BACKEND + '/csv/client', formData, {
            headers,
            observe: 'response'
        }).subscribe(response => {
        ...
        if (this.conflictedReplaceDataChoice.includes(null)) {
            ...
            for (let i = 0; i < this.conflictedData.length; ++i) {
                if (this.conflictedReplaceDataChoice[i]) {
                    this.dataToUpdate.push(this.conflictedData[i]['csv'] as ClientDataImportInterface);
                }
            }
    ...
    public insertData() {
        ...
        this.http.put(AppSettings.URL_BACKEND + '/csv/client', formData, {
            headers,
            observe: 'response'
        }).subscribe(response => {
