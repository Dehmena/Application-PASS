...
export interface InvoiceDataImportInterface {
    number: number;
    date: string;
    clientNumber: number;
    orderNumber: number;
...
@Component({
  selector: 'app-import-invoice-csv-dialog',
  templateUrl: './import-invoice-csv-dialog.component.html',
  styleUrls: ['./import-invoice-csv-dialog.component.css']
})
export class ImportInvoiceCsvDialogComponent implements OnInit {

    public csvFile: File;
    public dataToInsert: InvoiceDataImportInterface[] = [];
    public dataToUpdate: InvoiceDataImportInterface[] = [];
    ...
    public conflictedData: InvoiceDataImportInterface[] = [];
    ...
    constructor(public dialogRef: MatDialogRef<ImportInvoiceCsvDialogComponent>,
    ...
    processCsv() {
        ...
        this.http.post(AppSettings.URL_BACKEND + '/csv/invoice', formData, {
            headers,
            observe: 'response'
        }).subscribe(response => {
        ...
        if (this.conflictedReplaceDataChoice.includes(null)) {
            ...
            for (let i = 0; i < this.conflictedData.length; ++i) {
                if (this.conflictedReplaceDataChoice[i]) {
                    this.dataToUpdate.push(this.conflictedData[i]['csv'] as InvoiceDataImportInterface);
                }
            }
    ...
    public insertData() {
        ...
        this.http.put(AppSettings.URL_BACKEND + '/csv/invoice', formData, {
            headers,
            observe: 'response'
        }).subscribe(response => {
