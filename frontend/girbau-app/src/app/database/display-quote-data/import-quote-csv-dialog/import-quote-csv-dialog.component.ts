...
export interface QuoteDataImportInterface {
    number: number;
    date: string;
    totalAmount: number;
    clientNumber: number;
...
@Component({
    ...
    templateUrl: './import-quote-csv-dialog.component.html',
    styleUrls: ['./import-quote-csv-dialog.component.css']
})
export class ImportQuoteCsvDialogComponent implements OnInit {

    public csvFile: File;
    public dataToInsert: QuoteDataImportInterface[] = [];
    public dataToUpdate: QuoteDataImportInterface[] = [];
    ...
    public conflictedData: QuoteDataImportInterface[] = [];
    ...
    constructor(public dialogRef: MatDialogRef<ImportQuoteCsvDialogComponent>,
    ...
    processCsv() {
        ...
        this.http.post(AppSettings.URL_BACKEND + '/csv/quote', formData, {
            headers,
            observe: 'response'
        }).subscribe(response => {
        ...
        if (this.conflictedReplaceDataChoice.includes(null)) {
            ...
            for (let i = 0; i < this.conflictedData.length; ++i) {
                if (this.conflictedReplaceDataChoice[i]) {
                    this.dataToUpdate.push(this.conflictedData[i]['csv'] as QuoteDataImportInterface);
                }
            }
    ...
    public insertData() {
        ...
        this.http.put(AppSettings.URL_BACKEND + '/csv/quote', formData, {
            headers,
            observe: 'response'
        }).subscribe(response => {
